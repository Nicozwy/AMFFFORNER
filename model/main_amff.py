...
from masked_conv import masked_conv1d
...
handlers = [
    logging.FileHandler('results/_output.log'),#sev 2
...
# Attention
def scaled_dot_product_attention(q, k, v, d_key, dropout_rate=0.0):
    '''self-attention, Attention is all you need'''

    product = tf.matmul(a=q, b=k, transpose_b=True)
    weights = tf.nn.softmax(product)
    Z = tf.matmul(weights, v)

    hidden_size = q.get_shape().as_list()[-1]
    w_1 = tf.Variable(tf.random_normal([hidden_size, hidden_size], stddev=0.1))##attention_size = 200
    b_1 = tf.Variable(tf.random_normal([hidden_size], stddev=0.1))

    output = tf.tensordot(Z, w_1, axes=1) + b_1
...
def model_fn(features, labels, mode, params):
    ...
    char_embeddings2 = tf.reshape(output, [-1, dim_words, 50])

    # attention
    char_embeddings2, _ = attention(char_embeddings2, 200)
    ...
    word_embeddings = scaled_dot_product_attention(word_embeddings, word_embeddings, word_embeddings, params['dim'], 0)

    # # words 1d convolution
    word_embeddings2 = tf.layers.max_pooling1d(word_embeddings, 2, 1, padding='SAME')
    ...
    embeddings = tf.concat([a*word_embeddings,  b*char_embeddings, c*word_embeddings2, d*char_embeddings2], axis=-1)
...
if __name__ == '__main__':
    ...
    estimator = tf.estimator.Estimator(model_fn, 'results/saved_model', cfg, params)
